...
test('auther select from page', async ({ page }) => {
    ...
    await page.pause();
    await expect.soft(page.getByRole('combobox', { name: 'Author' })).toBeVisible();
    await page.getByRole('combobox', { name: 'Author' }).selectOption({ label: data.Fullusername });
    await page.getByRole('button', { name: 'Update' }).click();
});
test('auther select from page itslef', async ({ page }) => {
    await page.pause();
    await page.getByRole('link', { name: 'Pages', exact: true }).hover();
    //here we check Post tab visible 
    await expect.soft(page.getByRole('link', { name: 'Pages', exact: true })).toBeVisible();
...
test('Date select from page', async ({ page }) => {
    await page.pause();
    await page.getByRole('link', { name: 'Pages', exact: true }).hover();
    //here we check Post tab visible 
    await expect.soft(page.getByRole('link', { name: 'Pages', exact: true })).toBeVisible();
...
test('Date select from page itslef', async ({ page }) => {
    const date = new Date();
    await page.pause();
    await page.getByRole('link', { name: 'Pages', exact: true }).hover();
    //here we check Post tab visible 
